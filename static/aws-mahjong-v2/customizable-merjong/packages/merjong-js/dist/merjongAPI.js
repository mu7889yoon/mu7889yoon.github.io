...
const genRenderProfiles = (mpsz) => {
    const result = [];
    const numQtBuffer = []; // e.g.,　0,1',2",X,Q'
    let qtBuffer = []; // e.g., `'`, `"`
...
const genSVG = (renderProfiles, config) => {
...
const resolveBaseUrl = (designs, baseUrl) => {
    if (!baseUrl)
        return designs;
...
const getRenderConfig = (customConfig) => {
    ...
    const resolvedDesigns = resolveBaseUrl(customConfig.tileDesigns || {}, customConfig.baseUrl);
    return {
        tileDesigns: { ...theme.tileDesigns, ...resolvedDesigns },
        tileWidth: theme.tileWidth,
        tileHeight: theme.tileHeight,
        tileGap: theme.tileGap,
        spaceWidth: theme.spaceWidth,
