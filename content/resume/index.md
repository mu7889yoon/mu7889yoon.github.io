...
個人情報を扱うシステムでは、外部公開を前提としない閉域構成やログ保存期間を含む運用設計にも取り組んでいます。
...
解析した仕様をもとに、PHP / LaravelおよびAWSを用いてアプリケーションと実行基盤を再構築しています。

旧システムから新システムへのデータ移行では、既存データの構造や内容を確認しながら移行処理を実装し、本番切り替えに向けた検証を進めています。

個人情報を扱うため、外部公開を前提としない閉域構成とし、ログの保存期間を含む運用設計を行っています。

現在は本番切り替え前であり、移行および切り替えに向けた準備を進めています。
